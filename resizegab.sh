...
# Langkah 2: Gabungkan setiap dua file bagian atas menjadi satu halaman A4
a4_files=()
num_files=${#a5_files[@]}
for ((i=0; i<num_files; i+=2)); do
  if [[ -n "${a5_files[i+1]}" ]]; then
    a4_file="temp_a4/a4_$(printf "%02d" $((i/2+1))).pdf"
    pdfjam "${a5_files[i]}" "${a5_files[i+1]}" --nup 1x2 --papersize '{210mm,297mm}' --outfile "$a4_file"
    a4_files+=("$a4_file")
  else
    # Jika jumlah file ganjil, file terakhir tetap jadi A4 sendiri
    a4_file="temp_a4/a4_$(printf "%02d" $((i/2+1))).pdf"
    pdfjam "${a5_files[i]}" --papersize '{210mm,297mm}' --outfile "$a4_file"
    a4_files+=("$a4_file")
  fi
done
